/**
 * Interactive SQL Challenge Arena
 * LeetCode-style database problem-solving with automated assertions against the WASM engine.
 */

import { executeQuery } from '../db/engine.js';
...
ORDER BY TotalHours DESC;`,
    verify: (result) => {
      ...
      // Check that nulls are coalesced
...
ORDER BY SalaryGap DESC;`
...

  {
    ...
    description: `
      ...
      <p><strong>Task:</strong> In <code>DimCustomer</code>, write a query to identify all customers who have moved (more than 1 version record). Return their <code>CustomerId</code>, <code>CustomerName</code>, total number of historical revisions (<code>VersionCount</code>), and their current active postal code (<code>CurrentPostalCode</code>).</p>
    `,
    starterSql: `-- Challenge 4: Detect SCD Type 2 customer version revisions
SELECT 
    CustomerId,
    CustomerName,
    COUNT(CustomerSK) AS VersionCount,
    MAX(CASE WHEN IsCurrent = 1 THEN PostalCode END) AS CurrentPostalCode
FROM DimCustomer
GROUP BY CustomerId, CustomerName
HAVING COUNT(CustomerSK) > 1;`,
    verify: (result) => {
      if (!result.columns || result.columns.length < 4) {
        return { pass: false, error: 'Expected 4 columns: CustomerId, CustomerName, VersionCount, CurrentPostalCode.' };
      }
      if (result.rowCount !== 1) {
        return { pass: false, error: `Expected exactly 1 relocated customer (Contoso Retail Corp), received ${result.rowCount}.` };
      }
    ...
    hint: 'Group by CustomerId and CustomerName, apply HAVING COUNT(CustomerSK) > 1, and use conditional aggregation MAX(CASE WHEN IsCurrent = 1 THEN PostalCode END).',
    solution: `SELECT 
    CustomerId,
    CustomerName,
    COUNT(CustomerSK) AS VersionCount,
...
export function setupChallengesArena() {
  const challengeList = document.getElementById('challengeList');
  const titleEl = document.getElementById('challengeActiveTitle');
  const diffBadge = document.getElementById('challengeActiveDiff');
  const topicBadge = document.getElementById('challengeActiveTopic');
  const xpBadge = document.getElementById('challengeActiveXp');
  const descEl = document.getElementById('challengeActiveDesc');
  const editor = document.getElementById('challengeSqlEditor');
  ...
  const solutionBtn = document.getElementById('challengeSolutionBtn');
  const statusContainer = document.getElementById('challengeStatusContainer');
  const resultTableContainer = document.getElementById('challengeResultTable');
  const userXpBadge = document.getElementById('challengeTotalXp');

  if (!challengeList || !editor) return;

  let activeIndex = 0;

  // Load saved completed challenges & total XP from localStorage
  const completedStorageKey = 'omniflow_completed_challenges';
  let completedChallenges = JSON.parse(localStorage.getItem(completedStorageKey) || '[]');
  ...
  function updateXpDisplay() {
    ...
    if (userXpBadge) userXpBadge.textContent = `${totalXp} XP`;
  }

  function renderSidebar() {
    challengeList.innerHTML = CHALLENGES.map((ch, idx) => {
      const isCompleted = completedChallenges.includes(ch.id);
      const isActive = idx === activeIndex;
      const diffClass = ch.difficulty === 'Easy' ? 'badge-emerald' : ch.difficulty === 'Medium' ? 'badge-amber' : 'badge-rose';

      return `
        <div class="challenge-item ${isActive ? 'active' : ''} ${isCompleted ? 'completed' : ''}" data-idx="${idx}">
          ...
          <div class="challenge-item-meta">
            ...
            <span class="badge badge-purple">+${ch.xp} XP</span>
          </div>
        </div>
      `;
    }).join('');
  ...
  function selectChallenge(idx) {
    ...
    diffBadge.className = `badge ${ch.difficulty === 'Easy' ? 'badge-emerald' : ch.difficulty === 'Medium' ? 'badge-amber' : 'badge-rose'}`;
    topicBadge.textContent = ch.topic;
    xpBadge.textContent = `+${ch.xp} XP`;
    descEl.innerHTML = ch.description;

    editor.value = ch.starterSql;
    hintContainer.style.display = 'none';
    statusContainer.innerHTML = '';
    resultTableContainer.innerHTML = '<div style="padding: 24px; text-align: center; color: var(--text-muted);">Execute or Submit query to view output</div>';
  }
  ...
  if (testBtn) {
    testBtn.addEventListener('click', () => {
      ...
      renderOutputTable(res);

      statusContainer.innerHTML = `
        <div class="alert alert-info">
          ℹ️ Query executed in ${res.executionTimeMs} ms. Returned ${res.rowCount} rows. Click <strong>Submit Solution</strong> when ready to verify against test assertions!
        </div>
      `;
    });
  }
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', () => {
      const ch = CHALLENGES[activeIndex];
      const sql = editor.value.trim();
      if (!sql || !ch) return;
      ...
      if (res.error) {
        statusContainer.innerHTML = `
          <div class="alert alert-error">
            ❌ <strong>SQL Syntax / Runtime Error:</strong> ${escapeHtml(res.error)}
          </div>
        `;
        return;
      }

      const verification = ch.verify(res);
      if (verification.pass) {
        if (!completedChallenges.includes(ch.id)) {
          completedChallenges.push(ch.id);
          localStorage.setItem(completedStorageKey, JSON.stringify(completedChallenges));
          updateXpDisplay();
          renderSidebar();
        }
        ...
        statusContainer.innerHTML = `
          <div class="alert alert-success">
            🎉 <strong>Challenge Solved!</strong> All assertions passed successfully (+${ch.xp} XP awarded).
          </div>
        `;
      } else {
        statusContainer.innerHTML = `
          <div class="alert alert-error">
            ❌ <strong>Assertion Failed:</strong> ${escapeHtml(verification.error)}
          </div>
        `;
      }
    });
  ...
  if (solutionBtn) {
    solutionBtn.addEventListener('click', () => {
      ...
      if (confirm('Revealing the solution will show the reference T-SQL implementation. Continue?')) {
        editor.value = ch.solution;
        statusContainer.innerHTML = `
          <div class="alert alert-info">
            📖 Loaded reference solution into editor. Test or Submit to inspect the result!
          </div>
        `;
      }
    });
  ...
  function renderOutputTable(res) {
    if (res.error) {
      resultTableContainer.innerHTML = `
        <div style="padding: 16px; color: var(--accent-rose); font-family: var(--font-mono); font-size: 0.85rem;">
    ...
    if (!res.columns || res.columns.length === 0) {
      resultTableContainer.innerHTML = `
        <div style="padding: 16px; color: var(--accent-emerald);">
          ${escapeHtml(res.message || 'Query executed successfully with 0 rows returned.')}
        </div>
      `;
      return;
    }
    ...
    const tbody = res.values.map(row => {
      return `<tr>${row.map(v => `<td>${v === null ? '<span class="cell-null">NULL</span>' : escapeHtml(v)}</td>`).join('')}</tr>`;
    }).join('');
