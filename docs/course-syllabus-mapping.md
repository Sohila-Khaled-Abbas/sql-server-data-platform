...
| :--- | :--- | :--- | :--- |
| **CH01: Database Creation and Management** | Database creation scripts & wizards, physical files, secondary filegroups, integrity constraints, and database snapshots. | **Storage Engine Internals & Multi-Filegroup Topology**: Separation of metadata (`PRIMARY`), high-churn data (`DATA_FG`), non-clustered indexes (`INDEX_FG`), and partitioned cold storage (`ARCHIVE_FG`). Dynamic path auto-detection. Point-in-time copy-on-write sparse file snapshots. | - [`01_filegroups_and_files.sql`](file:///src/01_storage_and_schema/01_filegroups_and_files.sql)<br>- [`02_custom_types_and_rules.sql`](file:///src/01_storage_and_schema/02_custom_types_and_rules.sql)<br>- [`03_integrity_constraints.sql`](file:///src/01_storage_and_schema/03_integrity_constraints.sql)<br>- [`02_snapshot_lifecycle.sql`](file:///src/06_reliability_and_dr/02_snapshot_lifecycle.sql) |
