...
* 🌲 **[Clustered Index & B+Tree Traversal Live Telemetry (ITI)](docs/ch01-vid08-clustered-index-live.md)**: Live verification of `dbo.student` Primary Key Clustered Index, B+Tree physical hierarchy, Index Seek (`WHERE id = 804`), and non-indexed Scan bottlenecks on SQL Server 2022.
* 🔍 **[Non-Clustered Index & Key Lookup Live Telemetry (ITI)](docs/ch01-vid09-nonclustered-index-live.md)**: Live verification of Non-Clustered Index `i2` on `dbo.student(name)`, Key Lookup mechanics, Covering Index (`INCLUDE (age)`), and DMV introspection on SQL Server 2022.
* ⚡ **[SSMS Practical Indexing Lab Live Telemetry (ITI)](docs/ch01-vid10-demo-on-index-live.md)**: Live verification of single clustered index invariant (Msg 1902 rejection), secondary non-clustered index `i2` on `Student(st_fname)`, Index Seek + Key Lookup, and covering index (`INCLUDE`) on SQL Server 2022.
